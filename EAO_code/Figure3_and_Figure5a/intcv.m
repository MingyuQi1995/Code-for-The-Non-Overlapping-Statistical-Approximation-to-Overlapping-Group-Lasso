%Funtion of cross-validation for different methods.
...
%Output: lambda_min (the lambda with the samllest estimation error), lambda_min_pattern (the lambda with the samllest support recovery error)





function [lambda_min,lambda_min_pattern] = intcv(X,y,lambda_seq, opts, beta_real, type)
...
Deltapattern = [];
    ...
    if strcmp(type, '0')
       ...
       hamm = sum(beta_estpattern ~= beta_realpattern);

       Deltapattern(nlambda) = hamm/length(positions);
    ...
    for j = 1:(nlambda-1)
       ...
       Deltapattern(nlambda-j) = hamm/length(positions);
    end
    Delta_min = min(Delta);
